...
```
selection_ratio = clip(|Δ_s| + |Δ_d|, 0, Δ_CF) / Δ_CF · |⟨τ_x⟩_anom|
```

where `⟨τ_x⟩_anom` is the **anomalous** SC-induced orbital coherence from the off-diagonal BdG u·v amplitudes (output of `_compute_orbital_coherence_from_pairs`), not the general normal-state `⟨τ_x⟩_BdG`. This quantity is exactly zero at Δ=0 by construction.
...
where `⟨τ_x⟩_anom` is the anomalous SC-induced orbital coherence from `_compute_orbital_coherence_from_pairs` — not the general normal-state `⟨τ_x⟩_BdG`. It is exactly zero when Δ=0 or Q=0, enforcing the D₄h selection rule.
...
The bare susceptibilities χ₀(q) come from the Δ=0 BdG Hamiltonian via the Lindhard formula, implemented in the module-level `_lindhard_bubble()` function using `opt_einsum`. The Lindhard sum is split by Nambu sector pairs: `_NORMAL_SECTOR_PAIRS` (8 pairs: AA/BB particle and hole blocks, plus AB/BA cross terms) covers all normal-state contributions; `_ANOM_SECTOR_PAIRS` (4 particle↔hole pairs activated by Bogoliubov rotation) covers the SC-state anomalous cross-susceptibility χ_DQ^SC that is exactly zero at Δ=0 by the B₂g selection rule. The static Lindhard function is real by time-reversal symmetry — the imaginary part vanishes exactly at ω=0, so taking `chi0_tensor = chi0.real` after enforcing Hermiticity discards only numerical roundoff, not physical information. Projections:
...
```
χ_DQ_s_v = χ_DQ_s / (1 + |χ_DQ_s| / w),    w = _CHI_DQ_S_PADE_W = 0.05
```
...
**Correlation correction to Moriya damping:** the quasiparticle weight `z_qp = 1/r_J` (from cluster-ED) introduces an excess `r_J_excess = max(0, r_J − 1)`. Only the overcorrelated part (r_J > 1) boosts Γ_M — at most multiplying it by `_MAX_RENORM_SCALE = 2.9` — to prevent RPA runaway from vertex corrections that exceed the Gutzwiller-band picture. The clip has an asymmetric lower bound `(1 − 1/_MAX_RENORM_SCALE)` that permits a mild reduction below the bare value near the QCP. The `χ₀` bubble is left untouched: Ward identities require that the quasiparticle weight from the bubble (Z²) and the vertex correction (1/Z) cancel to Z, which is already encoded in the Gutzwiller-renormalized BdG bands feeding `χ₀`. A warning is logged when `z_qp < 0.35` (near Mott boundary) because `r_J_eff = 1/z_qp` is an unreliable proxy for the momentum-dependent quasiparticle weight in that regime.

**χ_SQ(q) full BZ scan:** `estimate_chi_SQ_q_full()` evaluates the spin–quadrupole cross-susceptibility χ_SQ(q) over a **24×24 q-grid** using `_lindhard_bubble` with `_NORMAL_SECTOR_PAIRS`. It performs two passes over the BZ — one at Δ=0 (normal-state eigenstates) and one at Δ≠0 (Bogoliubov eigenstates) — both using the same `_NORMAL_SECTOR_PAIRS` sector list. The SC pass uses the condensate band structure, which lifts the D₄h B₂g selection rule via Bogoliubov Γ₆/Γ₇ mixing in the normal-sector propagator, making χ_SQ_sc ≠ 0 even without explicit symmetry breaking.
...
**QCP sign-flip EMA guard:** when `|det_afm| < _DET_MARGINAL_FLOOR = 0.05` and `V_d_scalar` would flip sign relative to the cached value, the new value is blended with the old at weight `_EMA_NEW_WEIGHT = 0.25`. This suppresses numerical sign instabilities from tiny QCP boundary crossings without suppressing genuine physical sign changes.
...
**χ_SQ(q) BZ scan:** `estimate_chi_SQ_q_full` performs a dual 24×24 BZ scan for both the normal state (Δ=0, using normal eigenstates) and the SC state (Δ≠0, using Bogoliubov eigenstates on the same `_NORMAL_SECTOR_PAIRS`). The SC pass is skipped when `|Δ_s| + |Δ_d| < _QQ_DELTA_THRESH`. Broadening is physics-motivated: `η_n = max(_ETA_T_FRAC·kT, _ETA_GRID_FLOOR·t₀)` for the normal state; `η_sc = max(_ETA_DELTA_FRAC·|Δ|, _ETA_GRID_FLOOR·t₀)` for the SC state. Additional return quantities: `phi_d_overlap_n/sc` (d-wave form-factor overlap), `peak_region_n/sc` (AFM/antinodal/nodal/gamma classifier), `local_vertex_ok` (antinodal_frac_n > 0.5), `chi_SS_n/sc`, `psd_violations_n/sc`. In D₄h (Δ_ip=0) `chi_SQ_n = 0` exactly; the SC pass can lift this via Bogoliubov Γ₆/Γ₇ mixing.
...
**Susceptibility consistency:** χ₀ (normal state, Δ=0) is used for χ_DD_s, χ_DQ_s, χ_QD_s in the pairing vertex — feeding Δ≠0 susceptibilities back into the interaction would double-count the gap. `chi_QQ` (SC state, Δ≠0) is used exclusively for lattice stability diagnostics (G-matrix). The vertex is always built from the normal state: `solver._gap_amplitude = 0.0` is set before susceptibility evaluation. The `chi_QQ_bare` docstring now reads: "Bare JT orbital susceptibility: χ_QQ = −∂²Ω/∂Q² (eV/Å²) — not multiplied by g_JT²."
...
These signed quantities replace the earlier proportional-split approximation for `_lam_max_s` / `_lam_max_d` and are propagated through the output dict as `lambda_bare_s` / `lambda_bare_d` after applying the respective Gutzwiller factors `g_Delta_s` / `g_Delta_d`. When `V_d < 0` the channel weights are re-read from the 2×2 kernel cache (`vec_s`, `vec_d`) which already incorporate the sign of the d-wave interaction. Channel-specific Gutzwiller factors `g_Delta_s` and `g_Delta_d` are applied separately. A 2×2 pairing kernel matrix in (s, d) channel space is also constructed to guide the SCF update direction toward the dominant instability even when `|Δ| ≈ 0`.
...
- **Tc₂ (λ_max-BCS):** `Tc = _BCS_TC_PREFACTOR · ω_c · exp(−1/λ_max)` (1.13 × BCS weak-coupling) with cutoff `ω_c = max(t_eff, ω_JT)`. In the SC-triggered JT picture the JT phonon energy `ω_JT` sets the relevant boson scale when it exceeds the effective bandwidth.
...
Beyond BdG mean field, a 2-site (A–B) cluster is exactly diagonalized at each iteration. The cluster Hamiltonian now includes **two exchange channels**:
...
`J_bond_M_bare = effective_superexchange(g_J, tx_bare, ty_bare, doping)` is the A1g Gutzwiller-renormalised superexchange. `J_bond_Q_bare = J_CT · sinh(2Q/λ) · η_J` is the bare B1g orbital exchange (Z omitted: single bond); it is activated only when `|Q| > 1e-8`. `ClusterMF` receives `multi_op`, `B1g_op`, and `Z` as constructor arguments. The B1g orbital exchange uses `self.B1g_op` — the same SOC+CF eigenbasis operator as the BdG JT term `H_JT = g_JT · Q · B1g_op` — ensuring operator consistency between the cluster ED and the mean-field. The boundary Weiss field applies to the magnetic channel only (orbital order is encoded in Q).

**Two-channel Weighted Multivariate Linear Regression (WMLR):** after diagonalizing H_cluster, two independent vertex renormalizations are extracted as pure *fluctuation* vertices — free of static mean-field background (DMFT/DΓA double-counting artefact). The procedure has three steps:

**Step 1 — Weiss-field decontamination.** The boundary-field energy is subtracted state-by-state: `evals_int = evals − ⟨H_bound_full⟩_n`, so the residual energy variation is driven only by the intracluster exchange channels.

**Step 2 — MF background subtraction (y-axis cleaning).** The classical MF contribution from each channel is further removed:

```
evals_int_clean[n] = evals_int[n]
                   − J_bond_M_bare · ⟨O_A⟩_n · ⟨O_B⟩_n        [always]
                   − J_bond_Q_bare · ⟨B1g_A⟩_n · ⟨B1g_B⟩_n    [only if |Q| > 1e-8]
```

This makes the y-axis consistent with the x-axis (connected correlators), so the regression slope directly gives the pure fluctuation coupling `J_eff_fluct` without bias from the static AFM/orbital order.

**Step 3 — WMLR on cleaned quantities.** The Boltzmann weights are computed from `evals_int_clean` (shifted to non-negative), making the thermal weighting consistent with the regression target:

```
evals_int_clean ≈ const + J_M_fluct · corr_M + J_Q_fluct · corr_Q
```

where `corr_M = ⟨O_A⊗O_B⟩_n − ⟨O_A⟩_n·⟨O_B⟩_n` (connected correlator of `multi_op`) and `corr_Q = ⟨B1g_A⊗B1g_B⟩_n − ⟨B1g_A⟩_n·⟨B1g_B⟩_n` (connected correlator of `self.B1g_op`). Solved by centred WMLR with Bessel-corrected effective sample size `n_eff − 1`. When the orbital channel variance is insufficient (`S_QQ < min_var`) or the design matrix is collinear (`det < 1e-4 · S_MM · S_QQ`), the solver falls back to 1D magnetic-only regression with `r_Q = r_J`.

- **`j_renorm = r_J = J_M_fluct / J_bond_M_bare`** — A1g magnetic fluctuation vertex renormalization. Feeds back into the Moriya correction: `Γ_M = Γ_M_bare × clip(j_renorm, 1.0−1/_MAX_RENORM_SCALE, _MAX_RENORM_SCALE=2.9)`.
- **`q_renorm = r_Q = J_Q_fluct / J_bond_Q_bare`** — B1g orbital fluctuation vertex renormalization, extracted from `self.B1g_op` (same operator as BdG). Tracked via EMA (`_q_renorm_cur`, weight 0.35
Both ratios are clipped with data-adaptive upper bounds: `r_J ∈ [0.1, min(_MAX_REGR_CLIP=3.6, r_J_max)]` and `r_Q ∈ [−1.5, min(_MAX_REGR_CLIP, r_Q_max)]`, where `r_X_max = 1 + _REGR_SLOPE_LIMIT · |S_Xy| / (S_XX · X_bare)` limits the renormalization to the information actually contained in the variance.
...
The `/4` corrects for the 16-dimensional BdG space doubling. The d-wave projection is applied via an explicit `φ(k)` projection of `V(k−k')`. `V_d_scalar` is clamped to be non-negative; when fewer than 6 Fermi-surface points are available or no d-wave amplitude is present, `V_d_scalar = 0`. Both `V_s_scalar` and `V_d_scalar` are clamped to `[−V_cap, +V_cap]` before being stored in the vertex cache. `compute_gap_eq_vectorized` also performs an early-exit guard (`return` with zeros) when `_get_FS_points` returns fewer than 4 points, preventing downstream division-by-zero in the arc-length weight.
...
Module-level SCF constants (not in ModelParams):
    _NK = 64          # k-points per direction (must be even for commensurate q_AFM = (π,π)); parity is asserted at solver initialization.
...
Instance methods on RMFT_Solver (moved from module-level free functions):
...
Module-level Lindhard infrastructure:
    ...
    _ANOM_SECTOR_PAIRS    # 4 particle↔hole pairs for SC-state χ_DQ^SC (zero at Δ=0 by selection rule)
    _lindhard_bubble(sector_pairs, E_k, V_k, f_k, shift_idx, w, eta, fermi_fn)
                          # opt_einsum Lindhard sum; Hermitised ×0.25 (corrects for both Nambu sector doubling and Hermitisation); shared by all χ computations
...
RMFT_Solver
    ...
    │   │       returns: (chi_DD_s_moriya, chi_DQ_s, chi_QD_s, chi_QQ_tilde, Gamma_M_eff)  ← tuple
    │   └── get_susceptibilities_sc      SC-state χ_QQ via ∂²Ω/∂Q²
    ├── _get_chi0_norm_cache(...)        Δ=0 eigenvector cache across q-loop and iterations
    ├── _make_vertex_params(doping, tx, ty, g_t, J_eff, K_eff)  Γ_M_bare, V_JT, V_cap — shared by all RPA call sites
    ├── _unique_q_pairs(fs_pts)          deduplicated q=k_i−k_j with q↔−q canonical reduction; static method
    ├── _fs_arc_lengths(pts)             arc-length element dl_i per FS point (angle-sorted neighbours); static method
    │                                    used as proper FS integration measure: ∫…dS/|vF| → Σ dl_i/|vF_i|
    ...
    ├── solve_linearized_gap_equation(., g_t) λ_max, gap vector, arc-length weights, λ_JT_kernel, gap symmetry
    ├── compute_G_instability(δ, M,
    │       compute_dlambda=True,
    │       j_renorm=1.0)                G3 matrix, ∂λ_pair/∂Q, lambda_lin_max_q0 (at Q=0),
    │                                    PSD projection at q=0, χ_SQ(q) BZ scan (D₂h only)
    ├── estimate_chi_SQ_q_full(δ, M, Q,   dual BZ scan of χ_SQ(q) on 24×24 grid;
    │       μ, g_t, g_J,                  normal (Δ=0) and SC (Δ≠0) passes using
    │       Δ_s=0, Δ_d=0, n_q=24)         _NORMAL_SECTOR_PAIRS on respective eigenstates;
    │                                      returns chi_SQ_n/sc, chi_SS_n/sc, phi_d_overlap,
    │                                      antinodal_frac, peak_region, local_vertex_ok,
    │                                      psd_violations_n/sc, symmetry_ok, sc_computed
...
The vertex is always built from the normal state (Δ=0): there is no Δ-threshold invalidation. The cache stores `det_afm`, `det_q0`, `chi_DD_s_afm` (Moriya-damped χ_DD_s at q=(π,π)), `V_s_scalar`, `V_d_scalar`, `ansatz_unstable` (det_afm < 0), `Gamma_M_eff`, `j_renorm`, and FS geometry arrays `fs_pts`, `vF_arr`, `phi_d`. FS geometry is reused from cache when the vertex is still valid, avoiding redundant `eigh` calls.
...
5. Update `K_eff` on iteration 0 and when `|ΔQ| > _Q_THR_REL·λ_hop` (Å) **and** at least 5 iterations have passed since the last update with `|ΔM| > 0.02`, or `|Δj_renorm| > 0.05`, tracked via `_K_eff_last_Q`, `_K_eff_last_M`, `_K_eff_last_iter`, and `_last_j_renorm`.
6. Solve gap equations for (Δ_s_out, Δ_d_out) via RPA vertex fixed-point. Blend in 2×2 pairing kernel eigenvector direction (weight `_ALPHA_MIX_2X2 = 0.38`) to prevent channel locking.
...
The result dict includes: all converged order parameters, Hessian eigenvalues, G3-matrix diagnostics, λ_JT, λ_JT_sc, λ_JT_kernel, ∂λ_pair/∂Q, gap symmetry, channel decomposition, coherence length ξ/a, 2Δ₀/kTc, `chi_tau_sc`, `chi_tau_n`, `richardson_ok`, `selection_ratio`, `chi_DD_s` (Moriya-damped, at q=(π,π)), `chi_DQ_s`, `chi_QQ`, `rpa_factor`, `afm_unstable`, `ansatz_unstable`, `j_renorm`, `q_renorm`, `g_t`, `g_J`, `tx`, `ty`, `K_eff_scf`, `scf_dynamics_regime`, `mott_suspect`, `incommensurate_dq`, `incommensurate_chi_ratio`.
...
- S3 (w=0.180): `λ_JT > _DE_LAMBDA_JT_THRESH` (0.05) — SC-JT coupling above threshold
...
- *Tier 2 (smooth mechanism weights):*
  - `w_lJT`: parabolic arch on [0.05,1], plateau (=1) for λ_JT ∈ [0.32, 0.73]; unclipped maximum at λ_JT ≈ 0.52
...
| `u` | u | 30.0 | U/t₀ ratio; Hubbard U = u·t₀ |
| `lambda_soc` | λ_SOC | 0.190 eV | Atomic SOC constant (t₂g shell); determines Γ₆–Γ₇ splitting |
...
| `Delta_inplane` | Δ_ip | 0.011 eV | B₂g in-plane CF; splits Γ₇ doublet |
...
| `_BCS_TC_PREFACTOR` | 1.13 | BCS weak-coupling Tc = 1.13·ω_c·exp(−1/λ) |
...
| `_MU_LM` | 3.1 | LM regularization floor for M Newton step |
| `_ALPHA_HF` | 0.35 | Newton vs BdG fixpoint blend for M |
| `_FS_N_VERTEX` | 110 | FS k-points used in the vertex q-loop |
| `_Q_UPDATE_PERIOD` | 4 | Update Q every N inner iterations |
| `_ALPHA_MIX_2X2` | 0.38 | Blend weight: 2×2 eigenvector direction vs fixed-point gap update |
| `_MORIYA_C` | 0.21 | Moriya damping prefactor; α_M = C·f(δ)·sat(t/J), where f(δ)=δ/(δ+0.30) and sat(t/J)=(t/J)/(1+t/J) |
| `_ALPHA_MORIYA` | 0.02 | Moriya damping floor |
| `_LAMBDA_JT_VIABLE` | 0.05 | Minimum λ_JT_sc for SC-triggered JT viability |
| `_CHI_DQ_S_PADE_W` | 0.05 | Padé regularisation width for χ_DQ_s |
| `_ANDERSON_TIKHONOV` | 1e-8 | Tikhonov β / diag_max in Anderson normal equations |
| `_ANDERSON_TRUST` | 2.2 | Trust-region step-size limit (multiples of simple step) |
...
| `_RPA_V_CAP_ALPHA` | 2.5 | Dynamic vertex cap: V_cap = α·max(`_RPA_BW_FACTOR`·max(\|tx\|,\|ty\|), J_eff); 2.5× headroom above BEC-BCS crossover. No static 50 eV hard cap — cap is computed per call in `_make_vertex_params`. |
| `_DET_MARGINAL_FLOOR` | 0.05 | Below this \|det_afm\|, EMA smoothing is applied to V_d_scalar to suppress sign-flip noise near the QCP |
| `_MAX_REGR_CLIP` | 3.6 | Hard upper limit on regression renormalization factors r_J / r_Q |
| `_REGR_SLOPE_LIMIT` | 2.0 | Data-adaptive bound: r_max = 1 + REGR_SLOPE_LIMIT·\|S_My\|/(S_MM·M_bare); prevents extreme vertex enhancement |
...
| `_EMA_NEW_WEIGHT` | 0.25 | Unified EMA new-sample weight for j_renorm and Λ_inst; q_renorm uses 0.40 (faster orbital response) |
...
| `_DQ_FS_VERTEX` | 0.02 Å | Finite-difference step for ∂V(k,k')/∂Q on FS (≈2–4% of λ_hop) |
...
| `_DE_LAMBDA_JT_THRESH` | 0.05 | λ_JT below this → SC-JT window closed (S3 penalty threshold) |
| `_MATH_EPS` | 1e-9 | General protection floor against division by zero (replaces former `_KT_FLOOR = 1e-8`) |
...
| `_CHI_DQ_FLOOR_FRAC` | 1e-4 | Dynamic floor factor for χ_DQ_s: floor = _CHI_DQ_FLOOR_FRAC · √(χ_DD · χ_QQ) |
| `_MAX_RENORM_SCALE` | 2.9 | Upper clip for j_renorm in Moriya boost: Γ_M × clip(j_renorm, 1.0, 2.9) |
...
4. `compute_G_instability(j_renorm=result['j_renorm'])` at self-consistent M + `check_sc_jt_window()` with χ_τ_sc from post-SCF.
...
At convergence, the main SCF-RES line prints: converged order parameters (M, Q, |Δ_s|, |Δ_d|), density n, μ, F_bdg, j_renorm, q_renorm, det_AFM, JT flag (selection_ratio > 0.05), SCF dynamics regime (`converging` / `limit_cycle` / `first_order_jump` / `hysteretic`), and `⚠ ANSATZ UNSTABLE` if `det(RPA) < 0` was observed at any iteration. The correlation correction log line reads: `j_renorm=… q_renorm=…  [A1g vertex: Γ_M × clip(j_renorm, 1−1/2.9, 2.9)  |  χ₀ untouched (Gutzwiller bands)]`.

Additionally: channel decomposition (λ_s vs λ_d from signed Rayleigh quotients `lambda_bare_s/d`), λ_JT, λ_JT_sc, λ_JT_kernel, ∂λ_pair/∂Q, SC-triggered JT confirmation (hessian_lmin_sc < −kT), 2Δ₀/kTc, χ_τ breakdown (chi_tau_sc, chi_tau_n, δχ_τ, richardson_ok). SC-JT window diagnostics (K_eff, K_spont, K_SC, K_opt, K_distance, in_window, lambda_JT_sc, lambda_JT_opt). Incommensurate AFM scan result (dq_max/π, χ ratio, auto-retry outcome if triggered). `lambda_lin_max_q0` (linearised λ_max at Q=0). When `V_d < 0`, q-resolved vertex flags are appended: `V_afm_mean`, `V_fwd_mean`, `V_neg_frac`, `V_dd_fs`.
...
| `K_eff` update conditional | Updated when: first iteration, `Q` changes beyond `_Q_THR_REL · λ_hop` (Å), at least 5 iterations have passed **and** `\|ΔM\| > 0.02`, or `\|Δj_renorm\| > 0.05`. Back-action of Q on exchange rigidity approximate during SCF transient |
...
| j_renorm / q_renorm WMLR regression | Both vertex renormalizations are extracted as pure fluctuation vertices from a 2-site cluster: the MF background `J_bare·⟨O_A⟩·⟨O_B⟩` is subtracted from the energy before regression, and `self.B1g_op` (SOC+CF eigenbasis) is used consistently in both the cluster exchange term and the correlator. Near the Mott boundary (`j_renorm > _MAX_RENORM_SCALE = 2.9`) the Gutzwiller quasiparticle picture weakens and the momentum-dependent quasiparticle weight is not well captured. q_renorm (B1g channel) falls back to r_J when Q ≈ 0 or the orbital and magnetic correlators are collinear. r_Q is signed: negative values are permitted (clip lower bound −1.5) and can arise from JT back-action on the orbital exchange; values outside [−1.5, _MAX_REGR_CLIP] indicate numerical unreliability and are hard-clipped. |
